...
RELEASES_ROOT_DIR="$1"
RELEASE_NAME="$2"
FIRST_FILE_RELEASE_URL="$3"
SECOND_FILE_RELEASE_URL="$4"
...
if [ ! -d "$RELEASE_DIR" ]; then
    ...
    mkdir -p "$RELEASE_DIR"
    echo "releases directory $RELEASE_DIR created."
else
    echo "releases directory $RELEASE_DIR already exists."
fi
...
then
    ...
    for FILE_URL in "$FIRST_FILE_RELEASE_URL" "$SECOND_FILE_RELEASE_URL"
    do
